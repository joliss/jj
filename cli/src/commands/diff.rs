...
pub(crate) struct DiffArgs {
    ...
    /// heads and/or roots are supported, but gaps in the revset is not
...
pub(crate) fn cmd_diff(
    ...
    if args.from.is_some() || args.to.is_some() {
        ...
        let gaps_revset = workspace_command
            ...
            .evaluate()?;
        if let Some(commit) = gaps_revset.iter().commits(repo.store()).next() {
        ...
        let parents: Vec<_> = roots.iter().flat_map(|c| c.parents()).try_collect()?;
