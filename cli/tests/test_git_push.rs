...
fn test_git_push_sign_on_push() {
    ...
    test_env
        ...
        .success();
    test_env
        .run_jj_in(&workspace_root, ["new", "-m", "final commit"])
        .success();
    ...
    let output = test_env.run_jj_in(&workspace_root, ["git", "push", "-b", "bookmark2"]);
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    Changes to push to origin:
    ...
    // With with `behavior = "own"`, commit with different author should not be
    // signed
    let output = test_env.run_jj_in(
        &workspace_root,
        [
    ...
    test_env.add_config(
        r#"
    git.sign-on-push.behavior = "force"
    "#,
    );
    let output = test_env.run_jj_in(
        &workspace_root,
