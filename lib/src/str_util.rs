...
impl StringPattern {
    ...
    pub fn from_str_kind(src: &str, kind: &str) -> Result<Self, StringPatternParseError> {
        match kind {
            ...
            "substring" | "sub" => Ok(StringPattern::substring(src)),
            "substring-i" | "sub-i" => Ok(StringPattern::substring_i(src)),
